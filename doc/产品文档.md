...
### 1. 项目愿景
**DegenTempo** 旨在为 Farcaster 社交生态提供最顺滑的变现路径。用户无需持有 Gas 费（ETH），即可一键将社交收益（$DEGEN）转化为银行卡法币，通过 **LI.FI 聚合器**、**Tempo 链** 和 **Stripe** 实现价值闭环。

### 2. 核心业务逻辑 (The Core Loop)
1.  **身份层 (Auth):** 用户通过 Privy 登录 Farcaster。
2.  **钱包层 (Wallet):** 系统为用户生成 **Base 链智能合约钱包 (ERC-4337)**，并自动关联 **Tempo 链** 收款地址。
3.  **资金归集 (Funding - The "Magic" Step):**
    *   **问题**: 用户资金在 Farcaster 钱包 (EOA)，通常需要 ETH 才能转账。
    *   **解法**: 利用 **$DEGEN 的 EIP-2612 Permit** 功能。
    *   **流程**: 用户链下签署 Permit 消息 -> Smart Account 在一笔交易中同时执行 `permit()` + `transferFrom()` 把资金拉取到 AA 钱包。全过程由 Paymaster 代付，用户 EOA 无需 ETH。
4.  **聚合层 (Aggregation):** 集成 **LI.FI** 跨链聚合协议。
    *   自动寻找最佳路径：`Base (DEGEN) -> Bridge -> Tempo (USDC)`。
    *   **自动分润：** 协议层自动扣除 **1.5%** 平台手续费，直接打入平台金库，无需后端干预。
4.  **免 Gas 层 (Gasless):**
    *   用户签署 **UserOperation**。
...
*   **起点 (User Base Wallet):** 100% $DEGEN (在用户 Privy EOA 或外部 EOA 中)。
    *   *Action:* 用户签署 `Permit` 消息，授权 AA 钱包拉取资金。
*   **归集 (AA Wallet):** Smart Account 通过 UserOp 执行 `transferFrom` 将资金拉入。
*   **分流 A (Platform Revenue):** **1.5% $DEGEN** 直接由 LI.FI 智能合约划转至 DegenTempo 平台金库地址。
*   **分流 B (Bridge & Swap):** **98.5% $DEGEN** 进入跨链桥协议。
    *   在桥接过程中，资产被兑换为 **Tempo 链上的 USDC**。
*   **终点 (User Tempo Wallet):** 用户收到扣除手续费和桥接损耗后的 **USDC**。
*   **法币出口 (Stripe Payout):**
...
#### 3.1 账户与安全 (Privy + AA)
*   **社交登录：** 仅限 Farcaster 授权。
*   **智能钱包 (Smart Account)：**
    *   集成 Privy 嵌入式钱包作为签名者 (Signer)。
...
*   **手续费模型：** **1.5%** (通过 LI.FI `integrator_fee` 参数实现)。
*   **金库地址 (Treasury)：** LI.FI 合约自动将 1.5% 的 DEGEN 划转至平台收款地址。
*   **前端展示：**
    *   **Quote 预览**：显示“原始金额”、“平台服务费”、“桥接费”、“预估到账 USDC”。
    *   **Gas 状态**：显示 "Sponsored by DegenTempo"。

#### 3.3 交易执行 (Bridge & Swap)
*   **跨链协议：** 使用 **LI.FI SDK** (聚合 Across, Stargate 等)。
*   **关键技术：** 利用 **Permit (EIP-2612)** 实现 EOA -> AA 的无 Gas 资金转移。
*   **流程控制：**
    1.  **Get Quote**: 前端请求最优报价。
    2.  **Sign Permit**: 用户对 EIP-712 Permit 消息签名 (授权 AA 扣款)。
    3.  **Sign UserOp**: 用户对包含 `permit` + `transferFrom` + `bridge` 的 UserOp 签名。
    4.  **Execute**: Paymaster 提交上链。
    5.  **Track**: 前端轮询 LI.FI Status API 更新跨链进度。
...
*   **跨链失败：** 若 LI.FI 跨链在目标链执行失败，资金通常会退回到用户的源链地址或停留在桥的合约中。
    *   **处理：** 前端检测到状态为 `FAILED`，引导用户使用 LI.FI Explorer 进行手动挽救或联系客服。
*   **Stripe 充值未到账：**
...
### 1. 核心架构理念 (Serverless + Aggregator)
...
*   **协议聚合 (Protocol Aggregation):** 不自建合约，全部依赖 LI.FI 和 Alchemy 成熟的基础设施。
...
| **Cross-Chain** | **LI.FI SDK** | 聚合所有桥，提供最佳报价，支持 Integrator Fee。 |
...
#### 3.2 询价与交易 (Quote & Swap)
1.  **询价**: 前端调用 `lifi.getQuote()`。
    *   Params: `fromChain: Base`, `toChain: Tempo`, `token: DEGEN -> USDC`, `fee: 0.015`.
2.  **展示**: UI 显示包含手续费和 Gas 费（Sponsored）的最终报价。
3.  **签名**: 用户点击确认，Privy 弹出签名框 (Sign Message/UserOp)。
4.  **上链**: Alchemy Bundler 接收 UserOp，验证 Paymaster 签名，打包上链。
5.  **记录**: 交易成功后，前端将 `lifiTransactionId` 发送至 `POST /api/trade/record`。

#### 3.3 状态追踪与出金
1.  **轮询**: 前端使用 `lifi.getStatus(txId)` 轮询跨链状态。
2.  **到账**: 当状态变为 `DONE`，Tempo 钱包收到 USDC。
...
*   **功能**: 记录交易初始状态。
*   **输入**: `{ fid: string, lifiQuote: object }`
*   **逻辑**: 验证用户存在 -> 解析 Quote -> 创建 PENDING 状态的 Transaction 记录。

#### `GET /api/user/history`
*   **功能**: 获取用户交易历史。
*   **逻辑**: 查询 DB -> 针对 PENDING 的订单，实时调用 LI.FI API 更新状态 -> 返回前端。
...
*   **状态机同步 (State Machine Sync)：**
    *   **链上为准原则：** 数据库中的状态 (`PENDING`, `COMPLETED`) 仅作为缓存，最终状态必须以链上查询结果或官方 API (LI.FI/Stripe) 为准。
    *   **补偿机制 (Reconciliation)：**
        *   **GitHub Actions (免费版推荐)：** 配置 `.github/workflows/cron.yml`，每 10 分钟触发一次 `/api/cron/reconcile` 接口，实现 100% 最终一致性。详见《技术方案-定时任务(免费版).md》。
        *   **Webhook (推荐)：** 配置 LI.FI 和 Stripe 的 Webhook，被动接收状态变更通知，实时更新 DB。
*   **幂等性设计 (Idempotency)：**
    *   **交易提交：** `POST /api/trade/submit` 接口必须对同一 `lifiQuote.id` 进行去重检查，防止用户重复点击导致多次记录或重复执行。
    *   **出金请求：** Stripe Payout Session 的创建应基于唯一的业务 ID (`bizId`)，避免重复创建付款单。
...
*   **Paymaster 风控：**
    *   在 Alchemy Dashboard 配置严格的 **Gas Policy**，限制单次 Gas 上限（建议 <$0.50）和合约白名单（仅限 LI.FI Router）。
...
*   `NEXT_PUBLIC_LIFI_API_KEY`: LI.FI 跨链聚合。
...
2.  **滑点过高：**
    *   LI.FI 自动处理滑点保护。如果预估滑点超过 3%，前端提示用户确认。
3.  **跨链卡顿：**
    *   前端提供 LI.FI Explorer 链接，用户可自行查看跨链桥状态。
